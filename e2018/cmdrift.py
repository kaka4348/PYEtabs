def cmdrift(cdx, cdy):
    ...
    for i in range(len(data)):
        ...
        if data[i][FieldsKeysIncluded.index('Item')] == 'Diaph D1 X':
            ...
            cd = cdy
        alldrift = nn/cd
        if float(data[i][FieldsKeysIncluded.index('Avg Drift')]) <= alldrift:
            ok = 'ok'
        else:
            ok = 'not ok'
        p = [data[i][FieldsKeysIncluded.index('Story')], data[i][FieldsKeysIncluded.index('OutputCase')], data[i][FieldsKeysIncluded.index('Item')], data[i][FieldsKeysIncluded.index('Avg Drift')], '%.6f'%(alldrift), ok]
        drift.append(p)
        i += 1
    return drift
